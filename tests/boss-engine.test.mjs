...
// finish_boss (…08, …11), the current-fight reward split and base_max_hp
// (…15, kept by …16), no escape (…16) and the nightly reset's misses (which
// take nothing away since …14). Night Raids: tests/night-raid.test.mjs.
...
test("finish_boss only finishes an active boss, once", async () => {
  ...
  const again = (await finish(boss.id, "defeated", today)).rows[0].r;
...
test("epic bosses pay 100 gold", async () => {
  const { familyId, childIds: [kid] } = await makeFamily(db);
  const today = await londonToday(db);
  for (let i = 0; i < 8; i++) await finish((await activeBoss(db, familyId)).id, "defeated", today);
...
// --- The current fight (…15, kept by …16) and no escape (…16) -----------------
...
test("every activation sets active_since (first-time too); new bosses get base_max_hp = max_hp", async () => {
  ...
  assert.equal(swarm.current_hp, swarm.max_hp, "at full HP");
});

test("activation keeps a parent's HP edit on a queued boss (current_hp = max_hp)", async () => {
  const { familyId } = await makeFamily(db);
  const today = await londonToday(db);
  await db.query("update public.bosses set max_hp = 45, current_hp = 1 where family_id = $1 and name = 'Alarm Clock Swarm'", [familyId]);
  await finish((await activeBoss(db, familyId)).id, "defeated", today);
  const swarm = await activeBoss(db, familyId);
  assert.deepEqual([swarm.name, swarm.max_hp, swarm.current_hp], ["Alarm Clock Swarm", 45, 45]);
});

test("the reward counts only the current fight: damage logged before the boss's activation earns nothing", async () => {
  const { familyId, childIds: [a, b] } = await makeFamily(db, { children: 2 });
  const today = await londonToday(db);
  const slime = await activeBoss(db, familyId);
  // a's hit from an earlier fight of this boss (before its active_since).
  await db.query(
    "insert into public.boss_log (boss_id, event_type, amount, child_id, created_at) values ($1, 'damage', 30, $2, now() - interval '1 day')",
    [slime.id, a],
  );
  await tick(b, await quest(familyId, b, today, slime.current_hp)); // this fight: all b

  assert.equal((await bossById(slime.id)).status, "defeated");
  assert.equal((await stats(db, a)).gold, 0, "a's damage was in an earlier fight");
  assert.deepEqual((await log(slime.id, "gold_awarded")).map((r) => [r.child_id, r.amount]), [[b, 25]]);
});

test("no escape: finish_boss refuses 'escaped', activation refuses it too", async () => {
  const { familyId } = await makeFamily(db);
  const today = await londonToday(db);
  const boss = await activeBoss(db, familyId);
  await assert.rejects(finish(boss.id, "escaped", today), /invalid outcome/);
  await assert.rejects(
    db.query("select public.activate_boss($1, $2, 'escaped')", [familyId, today]),
    /invalid reason/,
  );
  assert.equal((await bossById(boss.id)).status, "active");
  assert.equal((await log(boss.id, "escaped")).length, 0);
});

test("a legacy escaped boss (dormant status) never comes back: activation only draws from the queue", async () => {
  const { familyId } = await makeFamily(db);
  const today = await londonToday(db);
  const slime = await activeBoss(db, familyId);
  await db.query("update public.bosses set status = 'escaped', retreats = 1, escaped_at = now() where id = $1", [slime.id]);
  const { rows } = await db.query("select public.activate_boss($1, $2, 'defeated') as id", [familyId, today]);
  const next = await activeBoss(db, familyId);
  assert.equal(next.name, "Alarm Clock Swarm");
  assert.equal(rows[0].id, next.id);
  assert.equal((await bossById(slime.id)).status, "escaped");
});
...
test("the reset marks past open quests missed and takes nothing away: no penalty, no party damage", async () => {
  const { familyId, childIds: [a, b] } = await makeFamily(db, { children: 2 });
  const boss = await activeBoss(db, familyId);
  ...
  const hpBefore = (await bossById(boss.id)).current_hp;
  ...
  assert.equal((await log(boss.id, "miss_penalty")).length, 0, "no boss-side penalty");
  assert.equal(await party(familyId), undefined, "the (dormant) party row isn't created or touched");
  const after = await bossById(boss.id);
  assert.equal(after.status, "active");
  assert.equal(after.current_hp, hpBefore, "a miss doesn't heal the boss either");
});

test("however much is missed, the boss never escapes", async () => {
  const { familyId, childIds: [kid] } = await makeFamily(db);
  const boss = await activeBoss(db, familyId);
  await db.query("update public.bosses set current_hp = 42 where id = $1", [boss.id]);
  await quest(familyId, kid, D, 500); // would once have knocked the party out

  const r = await reset(familyId, addDays(D, 1));

  const after = await bossById(boss.id);
  assert.deepEqual([after.status, after.current_hp], ["active", 42]);
  assert.equal(r.activated, null);
  assert.equal((await log(boss.id, "escaped")).length, 0);
  assert.equal(await party(familyId), undefined);
});
...
test("with no boss left, misses are still marked", async () => {
  const { familyId, childIds: [kid] } = await makeFamily(db);
  await db.query("update public.bosses set status = 'defeated' where family_id = $1", [familyId]);
