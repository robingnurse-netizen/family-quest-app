...
test("every frame of every character has shadow data", () => {
  const keys = ["hero", "rogue", "trash_bag_slime", "alarm_clock_swarm", "laundry_goblin", "cable_spider",
    "magma_behemoth", "chronosphinx", "abyssal_kraken", "shogun_bot"];
  for (const key of keys) {
    for (const [name, a] of Object.entries(read(key).animations)) {
      for (const frame of a.frames) {
        const s = a.shadow?.[frame];
        assert.ok(s && s.length === 4, `${key}/${name} ${frame}`);
        assert.ok(s[3] > 0, `${key}/${name}: contact band`);
        assert.ok(s[1] > 0 && s[1] <= a.width * 1.1 + 1, `${key}/${name}: width in the canvas`);
        assert.ok(s[2] >= 0, `${key}/${name}: lift`);
      }
    }
  }
});

test("shadows: on the ground standing and lying, lifted in a jump, wide lying down", () => {
  const lifts = (a) => a.frames.map((f) => a.shadow[f][2]);
  assert.ok(lifts(rogue.idle).every((l) => l === 0));
  assert.ok(lifts(rogue.pounce).slice(3, 7).every((l) => l > 0), "airborne pounce frames");
  assert.ok(lifts(hero.victory).some((l) => l > 5), "the hero's victory jump");
  const width = (a, i) => a.shadow[a.frames.at(i)][1];
  assert.ok(width(hero.ko, -1) > 2 * width(hero.ko, 0), "the hero lying flat casts a long shadow");
  assert.ok(width(rogue.ko, -1) > width(rogue.idle, 0), "so does Rogue");
  assert.ok(lifts(hero.ko).at(-1) === 0 && lifts(rogue.ko).at(-1) === 0, "lying on the ground");
});

test("shadows cover the contact band: Rogue's far (hind) paws, drawn higher, are in it", () => {
  // His idle: the near front paw reaches the ground line; the hind paws end
  // higher (three-quarter view). The band reaches up to them.
  const [, , , band] = rogue.idle.shadow[rogue.idle.frames[0]];
  assert.ok(band >= 5, `band ${band}px`);
  // Airborne frames keep the grounded band (their shadow stays on the ground).
  const pounce = rogue.pounce;
  for (const i of pounce.airborne) assert.equal(pounce.shadow[pounce.frames[i]][3], pounce.shadow[pounce.frames[0]][3]);
});
